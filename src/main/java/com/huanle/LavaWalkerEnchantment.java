...
public class LavaWalkerEnchantment extends Enchantment {
    ...
    public static class LavaWalkerHandler {
        ...
        public static void onLivingUpdate(LivingEvent.LivingTickEvent event) {
            ...
            System.out.println("[LavaWalker] onLivingUpdate: " + entity.getName().getString() + ", enchantLevel=" + enchantLevel);
            if (enchantLevel > 0) {
                try {
        ...
        public static void onWorldTick(TickEvent.LevelTickEvent event) {
            if (event.phase == TickEvent.Phase.END && !event.level.isClientSide) {
                System.out.println("[LavaWalker] onWorldTick triggered");
        ...
        private static void freezeLava(LivingEntity entity, Level level, BlockPos pos, int enchantLevel) {
            ...
            System.out.println("[LavaWalker] freezeLava called for: " + entity.getName().getString() + ", pos=" + pos + ", enchantLevel=" + enchantLevel);
                        ...
                        if (blockState.is(Blocks.LAVA) && blockState.getValue(LiquidBlock.LEVEL) == 0) {
                            ...
                            if (level.isEmptyBlock(abovePos) || level.getBlockState(abovePos).getCollisionShape(level, abovePos, CollisionContext.empty()).isEmpty()) {
                                ...
                                if (currentState.is(Blocks.LAVA) && currentState.getValue(LiquidBlock.LEVEL) == 0) {
                                    // 将熔岩转换为岩浆块并更新周围方块
                                    BlockState obsidianBlock = Blocks.OBSIDIAN.defaultBlockState();
                                    level.setBlock(mutablePos, obsidianBlock, 3);
                                    level.neighborChanged(mutablePos, Blocks.OBSIDIAN, mutablePos);
        ...
        private static void updateLavaBlocks(ServerLevel level) {
            try {
                ...
                while (iterator.hasNext()) {
                    ...
                    System.out.println("[LavaWalker] Checking block at " + pos + ", state: " + currentState);
                    if (timeLeft <= 0) {
                        // 恢复为熔岩
                        if (level.isLoaded(pos)) {
                            if (currentState.is(Blocks.OBSIDIAN)) {
                                // 设置为熔岩并更新周围方块
                                ...
                                System.out.println("[LavaWalker] Obsidian at " + pos + " reverted to lava.");
                            }
                        }
                        iterator.remove();
                    } else {
